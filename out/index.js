...
bot.onText(/^\/getFolder/i, (msg) => {
    if (msgTools.isAuthorized(msg) < 0) {
        ...
        sendMessage(msg, '<a href = \'' + driveUtils.getFileLink(constants.GDRIVE_PARENT_DIR_ID, true) + '\'>Drive mirror folder</a>', 60000);
    }
});
...
function cancelMirrorForChat(chatId) {
    var count = 0;
    dlManager.forEachDownload(dlDetails => {
...
function sendMessageReplyOriginal(dlDetails, message) {
...
function editMessage(msg, text, suppressError) {
